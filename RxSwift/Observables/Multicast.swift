...
extension ObservableType {
     ...
     */
    public func multicast<Subject: SubjectType, Result>(_ subjectSelector: @escaping () throws -> Subject,
                                                        selector: @escaping
                                                        (Observable<Subject.Element>) throws -> Observable<Result>)
...
: ConnectableObservable<Subject.Element> {
    ...
    override func connect() -> Disposable {
        return self.lock.performLocked {
            ...
            let connection = Connection(parent: self,
...
, ObserverType where ConnectableSource.Element == Observer.Element {
    ...
    func run() -> Disposable {
        ...
        if self.parent.count == 0 {
            self.parent.count = 1
            // 第一次的时候, 记录了 source Connec 的 subscription.
